...
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
    ...
    private V doGet(Object key) {
        ...
        Comparator<? super K> cmp = comparator;
        outer: for (;;) {
            // 1. 先找到最底层中，小于这个key的最大节点 ，这里从最高级index开始寻找，以skiplist的方式
            // 然后从b的下一节点开始遍历（正常>=key）
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                Object v; int c;
                if (n == null)
                    break outer;
                ...
                // n的key > 当前key，就是key不存在
                if (c < 0)
                    break outer;
                /**
                 * 2. 可能被修改了，向下一个节点遍历继续判断
                 */
                b = n;
                n = f;
            }
        ...
        outer: for (;;) {
            ...
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                if (n != null) {
                    ...
                    if (n != b.next)               // inconsistent read
                        break;
                    // n的值等于null，正在删除，执行剩下删除，然后进入下次outer的循环
                    if ((v = n.value) == null) {   // n is deleted
                        n.helpDelete(b, f);
                        break;
                    }
                    ...
                    if (b.value == null || v == n) // b is deleted
                        break;
                    // 当前key比n的大（就是b后有插入数据），进入循环，遍历到下个节点
                    if ((c = cpr(cmp, key, n.key)) > 0) {
                        b = n;
                        n = f;
                        continue;
        ...
        outer: for (;;) {
            ...
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                ...
                if (!n.casValue(v, null))
                    break;
                /**
                 * 给当前节点的后面新增一个marker节点，然后把前一个节点的下个节点更新为原来的下一个节点（把当前节点已经删除）
                 */
                if (!n.appendMarker(f) || !b.casNext(n, f))
                    ...
                    /**
                     * 从链表删除节点后，清理index
                     * 通过重新遍历查找Predecessor，来删除index
                     */
                    findPredecessor(key, cmp);      // clean index
                    /**
                     * 如果最高层级index，没有index，尝试减少层级
                     * 需要至少4级，且当前head的连续3个层级都是没index，才会把head为原来的下一层（就删除head那层）
                     */
                    if (head.right == null)
                        tryReduceLevel();
                }
    ...
    private void tryReduceLevel() {
         ...
         * d：当前head的下一层
         * e：当前head的下2层
         * 并且这三层都是没有index，才会删除，更新head为d（就是原head的下一层）
         */
        if (h.level > 3 &&
