...
public abstract class AbstractQueuedSynchronizer
    ...
    public final boolean hasQueuedPredecessors() {
        // 就是判断调用方法时，队列中是否有线程在当前线程前面（不包括队列中的第一个）
        ...
        Node h = head;
        Node s;
        // h=t , 可能没有，或者只有一个元素，而第一个元素就是已完成，所以就是没等待的线程
        // h.next == null: 原来的第二个已完成，证明第二个肯定不是当前线程（不然进来这），所以在调用方法时，肯定有前面的线程
        // s.thread != Thread.currentThread()：队列中有比当前线程旧的等待线程，所以肯定有
        return h != t &&
